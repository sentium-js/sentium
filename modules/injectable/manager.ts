...
export class InjectableManager<Target extends Class> {
  ...
  declare<Injects extends readonly Class[]>(
    injects: Injects,
    initializer?: InjectableInitializer<InstanceTypes<Injects>, InstanceType<Target>>
  ): void {
   ...
   */
  resolve(scope: InjectableScope = defaultInjectableScope): InstanceType<Target> {
  ...
  async resolveAsync(
    ...
    if (this.metadata.initializer) {
      ...
      if (this.metadata.asyncResolves.get(scope)) {
        return this.metadata.asyncResolves.get(scope) as Promise<InstanceType<Target>>;
      }
